        ...
        cyg_uint8 *pstate, struct dhcp_lease *lease)
{
    ...
    unsigned short bcm_sock_opt;    // cliffmod
...
#define CHECK_XID() (  /* and other details */                                  \
    ...
    // begin cliffmod
#ifdef MULTI_INF_SAME_SUBNET
    // set up Broadcom defined socket option bits which identify
    // network interface.
    bcm_sock_opt = BcmNetIfNameToSockOpt( intf );
    if (setsockopt(s, SOL_SOCKET, bcm_sock_opt, &one, sizeof(one))) {
        perror("setsockopt SO_REUSEPORT");
        return false;
    }
#endif
